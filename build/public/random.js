  ...
  for (let i = 0; i < content.length; i++) {
    switch (type) {
      ...
      case 2:
        ...
        if ((content[i] == "A" || content[i] == "B" || content[i] == "C" || content[i] == "D") && i > 10 && content[i + 1] != " " && format.test(content[i + 1])) {
          arrayInd.push(i);
        }
        break;
    }
...
function RandomEachQuestion() {
  if (lsAnswer == 0) {
    alert("List Answer is not exists.Please import list answer before random question. Thanks");
    return;
  }
  if (lsQuestion == 0) {
...
function ShowAnswer() {
  ...
  if (lsHaveAnswer.length == 0) {
    alert("List is empty! Please choose answer for question. And retry click button again Thanks");
    return;
  }
  let totalCorrect = 0;
  let totalWrong = 0;
  ...
  lsHaveAnswer.forEach(item => {
    ...
    if (checkCorrect) totalCorrect += 1;else totalWrong += 1;
    lsAnswerStr = `${lsAnswerStr} </br> ${item.ContentQs.question}  
            ...
            </br> ${item.ContentQs.lsAnswer[3] ? item.ContentQs.lsAnswer[3] : ""} 
            </br> <b>Your answer</b>: <span class='${!checkCorrect ? "red" : "green"}'>${item.answer} </span>, <b>Right answer: ${answerR.answer}</b> <br/> ${answerR.explan}<hr>`;
  });
...
function AddnewQuestion() {
  ...
  if (dataInp) {
    ...
    for (let i = 0; i < lsNumber.length; i++) {
      var content = dataInp.slice(dataInp.indexOf(lsNumber[i]), dataInp.indexOf(lsNumber[i + 1]));
...
function AddAnswer() {
  ...
  if (dataInp) {
    ...
    for (let i = 0; i < indAnsers.length; i++) {
      var content = dataInp.slice(dataInp.indexOf(indAnsers[i]), dataInp.indexOf(indAnsers[i + 1]));
